...
ws = workstation("/mnt/SSDCashe/WS/MAKG_v3/")  # set the execution path
...
introduced_variable = variable(id = None
                               ...
                               , optional_selector= '[VARIABLE_NAME] magp:level "1"^^xsd:integer.')
...
exec = query_execution.queryExecutor(endpoint=config_frame.iloc[0].endpoint, query=b.compile(), min_supp=config_frame.iloc[0].min_supp,  format=config_frame.iloc[0].format, timeout=config_frame.iloc[0].timeout)
