...
def execute_task(timeout):
    ws = workstation("/mnt/SSDCashe/WS5/MAKG_" + str(timeout) + "/")    # set the execution path
    endpoint = 'http://ma-graph.org/sparql'  # set the queried endpoint
    max_itemset_len = 20
    min_supp = 100
    format = 'csv'
    timeout = timeout
    current_header = makg_header
    ...
    counting_variable = variable(id=None,
                                 ...
                                 optional_selector='[VARIABLE_NAME] prism:publicationDate [VARIABLE_NAME]_pubdate. \n FILTER([VARIABLE_NAME]_pubdate >= "2015-01-01T00:00:00+00:00"^^xsd:dateTime).')

    introduced_variable = variable(id=None
                                   , name='extend'
                                   , type='magc:FieldOfStudy'
                                   , unique_property_selector=None
                                   , unique_property_value=None
                                   , counted_variable=True
                                   , counted_variable_id=0
                                   , optional_selector='[VARIABLE_NAME] magp:level "2"^^xsd:integer.')
    # Set it

    base_connector = variable_connector(counting_variable, introduced_variable, 'fabio:hasDiscipline')
    ...
    exec = query_execution.queryExecutor(endpoint=config_frame.iloc[0].endpoint, query=b.compile(),
    ...
    print('First execution is ready')
    ...
    for itemset_len in range(1, max_itemset_len):
        print(str(mp.current_process().name), " ", timeout, ' >> current iteraton: ', itemset_len)
        ...
        for idx, row in df.iterrows():
            ...
            for col in list(df.columns[:-1]):
                ...
                cstatement.connectors.append(
                    variable_connector(counting_variable, fixed_variable, base_connector.connector))
            ...
            cstatement.connectors.append(
                variable_connector(counting_variable, introduced_variable, base_connector.connector))

            with open(ws.dir_queued_queries + str(itemset_len) + '_' + str(idx) + '.rq', 'w', encoding='utf-8') as out:
                out.writelines(cstatement.compile())

            df_extending.to_csv(ws.dir_queued_queries + str(itemset_len) + '_' + str(idx) + '.info', index=False,
                                encoding='utf-8')
        ...
        if (previous_max_length == df.itemset_length.max()):
            print(str(mp.current_process().name), " ", timeout, ' >> [STOPPING] Cannot extend anymore, this was the final extension')
            break
...
def main():
    ...
    timeouts = list(np.arange(10000, 350000, 10000))
    results = [pool.apply_async(execute_task, args=(to, )) for to in timeouts]
