...
def execute_task(timeout):
    ws = workstation("/mnt/SSDCashe/WS3/DBpedia_" + str(timeout) + "/")  # set the execution path

    endpoint = 'http://dbpedia.org/sparql'  # set the queried endpoint
    max_itemset_len = 20  # Set max iteration
    min_supp = 10000  # Set min supp
    format = 'csv'  # Don't touch it, except endpoint wont provide CSV
    timeout = timeout  # Timeout
    current_header = dbpedia_header  # Change this
    ...
    introduced_variable = variable(id=None
                                   , name='extend'
                                   , type=None
                                   , unique_property_selector=ontology_attribute_template
                                   , unique_property_value="yago"
                                   , counted_variable=True
                                   , counted_variable_id=0
                                   , optional_selector='')
    # Set it
    base_connector = variable_connector(counting_variable, introduced_variable, 'rdf:type')
...
def main():
    ...
    timeouts = list(np.arange(10000, 150000, 10000))
